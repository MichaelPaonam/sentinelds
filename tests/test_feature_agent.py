...
class TestFeatureToolsFunctionality(unittest.TestCase):
    ...
    def test_csv_read_success(self) -> None:
        ...
        self.assertEqual(
            res["columns"], ["eye_aspect_ratio", "yawn_count", "head_pose_angle", "label"]
        )
    ...
    def test_find_files_success(self) -> None:
        """Verifies find_files correctly lists and patterns matching files."""
        # Clean test and missing test were created in setUp inside self.temp_dir.name
        res = find_files(self.temp_dir.name, "csv")
        self.assertEqual(res["status"], "success")
        self.assertEqual(res["pattern"], "*.csv")
        self.assertEqual(res["count"], 2)

        filenames = [f["name"] for f in res["files"]]
        self.assertIn("clean_test.csv", filenames)
        self.assertIn("missing_test.csv", filenames)

        # Try with a pattern
        res_pattern = find_files(self.temp_dir.name, "clean_*.csv")
        self.assertEqual(res_pattern["status"], "success")
        self.assertEqual(res_pattern["pattern"], "clean_*.csv")
        self.assertEqual(res_pattern["count"], 1)
        self.assertEqual(res_pattern["files"][0]["name"], "clean_test.csv")

    def test_find_files_not_found(self) -> None:
        """Verifies find_files returns error dict for invalid directories."""
        res = find_files("/nonexistent/directory_path")
        self.assertEqual(res["status"], "error")
        self.assertIn("Directory not found", res["error"])
